...
PLUGIN = "yandex_calendar"
PROJECT = "hermes-yandex-calendar"
REQUIRED_ENV = ("YANDEX_CALENDAR_LOGIN", "YANDEX_CALENDAR_APP_PASSWORD")

#: What an install gives the agent before YANDEX_CALENDAR_ACTIONS says
#: otherwise: every tool, since the default stays ``all`` for compatibility.
DEFAULT_TOOLS = {
...
DROPIN_INSTALL = "unzip hermes-yandex-calendar-plugin-<version>.zip -d ~/.hermes/plugins/"
ENABLE = "hermes plugins enable yandex_calendar"
ADD_CREDENTIALS = (
...
ROOT_INSTALL = GIT_INSTALL.replace("/hermes_yandex_calendar ", " ")

#: Answers typed at the credential prompts of the Git install. Nothing here
#: ever reaches Yandex: no test calls a tool.
PROMPT_ANSWERS = {
    "YANDEX_CALENDAR_LOGIN": "install-check@example.invalid",
    "YANDEX_CALENDAR_APP_PASSWORD": "install-check-not-a-password",
}
...
def test_readme_gives_the_commands_under_test() -> None:
    ...
    commands = (GIT_INSTALL, PYPI_INSTALL, DROPIN_INSTALL, ENABLE, ADD_CREDENTIALS, ADD_DEFUSEDXML)
...
def test_dropin_archive_loads(home: Home) -> None:
    ...
    home.run(ENABLE)
    home.run(ADD_CREDENTIALS)
    # The archive brings no dependencies; a standard Hermes already has them.
    _assert_loaded(home, source="user")
